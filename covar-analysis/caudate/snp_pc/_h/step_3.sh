...
exec > >(tee -a "$LOG_DIR/extract_vmr_${CHR}_out.log")
exec 2> >(tee -a "$LOG_DIR/extract_vmr_${CHR}_err.log" >&2)
...
conda run -p $ENV_PATH/epigenomics Rscript ../_h/03.extract_vmr.R $CHR
if [ $? -ne 0 ]; then
