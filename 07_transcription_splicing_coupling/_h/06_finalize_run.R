#!/usr/bin/env Rscript
#### 07_transcription_splicing_coupling -- seal the run ####
##
...
run_dir <- file.path(repo_root(), "07_transcription_splicing_coupling", "_m",
                     "runs", opts$run_id)
if (!dir.exists(run_dir)) stop("No such run: ", run_dir)

dec_f <- file.path(run_dir, "results", "coupling-decision.tsv")
...
append_manifest(list(dir = run_dir), list(
    ...
    n_significant = dec$n_significant[1],
    coupling_supported = dec$coupling_supported[1],
    git_commit = git_commit(), git_dirty = as.character(git_dirty()),
    sealed_at = format(Sys.time(), "%Y-%m-%dT%H:%M:%S%z")))
close_run(list(dir = run_dir))
message("[07] sealed ", opts$run_id, " with decision ", dec$decision[1])
