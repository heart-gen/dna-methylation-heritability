...
out_path <- here("simulation-analysis/comparison/summary_counts/r2_testing/_m")
...
enet_all <- enet_all %>%
  mutate(method = "Elastic Net", N = as.character(N),
         h2_category = case_when(
           r_squared_cv <= r2_thresh ~ "Low prediction",
           h2_unscaled < 0.1 & r_squared_cv > r2_thresh ~ "Non-heritable",
           h2_unscaled >= 0.1 & r_squared_cv > r2_thresh ~ "Heritable"
         ))
...
# Save plot
plot_file <- file.path(out_path, paste0("simulated_data_stacked_", r2_thresh))
