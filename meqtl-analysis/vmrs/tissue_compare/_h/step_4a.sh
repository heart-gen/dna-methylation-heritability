#!/bin/bash
#SBATCH --account=p32505
#SBATCH --partition=short
#SBATCH --job-name=allpairs_run_mash
#SBATCH --mail-type=FAIL ## If you want to have it email you for any reason
#SBATCH --mail-user=elisajohnson2027@u.northwestern.edu ## replace with your email
#SBATCH --nodes=1
#SBATCH --cpus-per-task=1
#SBATCH --mem=10gb
#SBATCH --array=1-500%200
#SBATCH --output=logs/allpairs.run_mash.%A_%a.log
#SBATCH --time=01:00:00

# Function to echo with timestamp
log_message() {
    echo "$(date '+%Y-%m-%d %H:%M:%S') - $1"
}

log_message "**** Job starts ****"

log_message "**** Quest info ****"
echo "User: ${USER}"
echo "Job id: ${SLURM_JOBID}"
echo "Job name: ${SLURM_JOB_NAME}"
echo "Node name: ${SLURM_NODENAME}"
echo "Hostname: ${HOSTNAME}"
echo "Task id: ${SLURM_ARRAY_TASK_ID:-N/A}"

## List current modules for reproducibility
log_message "**** Loading modules ****"

module purge
module list

# Set path variables
log_message "**** Loading conda environment ****"

source /projects/p32505/opt/miniforge3/etc/profile.d/conda.sh
conda activate /projects/p32505/opt/envs/rnaseq

MODEL="local"
OUTDIR="${MODEL}/temp"
mkdir -p ${OUTPUT}

Rscript ../_h/04.allpairs_mash_model.R --chunk ${SLURM_ARRAY_TASK_ID} \
        --model $MODEL --output $OUTDIR

if [ $? -ne 0 ]; then
    log_message "Error: Conda or script execution failed"
    exit 1
fi

conda deactivate

log_message "**** Job ends ****"
