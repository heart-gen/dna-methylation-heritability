...
filter_pheno <- function(BSobj, pheno_file_path, sample_blacklist = NULL) {
    ...
    pheno_filtered <- pheno %>%
        filter(agedeath >= 17, region == "hippocampus")
...
write_covar <- function(BSobj, pheno, id, meth_merged, out_covs) {
    out_cov  <- file.path(out_covs, "TOPMed_LIBD.covar")
    out_qcov <- file.path(out_covs, "TOPMed_LIBD.qcovar")
                                        # Filter data
...
assays_dir <- here("inputs/wgbs-data/hippocampus/_m", 
                   paste0("hippocampus_chr", chr, "_BSobj"))
BSobj <- loadHDF5SummarizedExperiment(dir = assays_dir)

output_path <- here("vmr-analysis", "all_individuals", "hippocampus", "_m")
subdirs <- c("covs", "cpg")
...
sample_blacklist <- fread(here("vmr-analysis/all_individuals/hippocampus/_m/samples_blacklist.txt"), 
                          header = FALSE)[[1]]
