...
writeLines(c(
    ...
    "",
    "  THE THREE LOCKED CRITERIA (PI 2026-09-18). (i) and (ii) are the primary",
    "  and are evaluated on the shared caudate loci alone; (iii) is separate.",
    "    (i)   all three replicates attenuate in the same direction;",
    "    (ii)  mean relative attenuation A >= 0.10, where",
    "          A = mean_r (R2_full - R2_n118,r) / R2_full;",
    "    (iii) gap_closed >= 0.50 -- SEPARATE, and the only criterion that",
    "          licenses calling donor count a plausible major contributor to the",
    "          caudate-DLPFC difference specifically.",
    "  None of these is a significance cutoff. 0.10 on the replicate spread is an",
    "  AGREEMENT TOLERANCE, 0.10 on A is a MAGNITUDE requirement, and 0.50 is an",
    "  INTERPRETIVE threshold. An effect-size criterion was chosen over a paired",
    "  test deliberately: with ~11k paired loci a trivial attenuation would be",
    "  'significant' and would still say nothing about whether donor count",
    "  matters.",
    "",
    "  UNCERTAINTY ON A IS REPORTED, NOT GATED. A delete-one-chromosome weighted",
    "  block jackknife gives its CI; loci within a chromosome are not",
    "  independent, so the block is the chromosome. Statistical significance is",
    "  deliberately not a further hard gate for this tier.",
    "",
    ...
    "",
    "  This shift is reported ALONGSIDE the attenuation and is NEVER folded into",
    "  the 0.10 threshold: A is computed on prediction r2 alone and carries no",
    "  boundary-rate correction. That is what keeps overall prediction",
    "  attenuation distinguishable from the accompanying loss of estimator",
    "  resolution, rather than silently mixing the two into one number.",
    "",
