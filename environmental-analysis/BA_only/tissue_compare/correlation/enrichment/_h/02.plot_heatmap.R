...
plot_all_env_heatmap <- function(w = 8, h = 10){
  ...
  save_plot(p, "all_env_heatmap", w, h)
}

## Main
envs <- c("smoking", "less_than_hs", "more_than_hs", "single",
          "previously_married", "codeine", "morphine", "cocaine", 
          "ethanol", "nicotine", "amphetamines")

for (label in envs){
  plot_tile(label, 10, 6)
  plot_all_env_heatmap()
}
