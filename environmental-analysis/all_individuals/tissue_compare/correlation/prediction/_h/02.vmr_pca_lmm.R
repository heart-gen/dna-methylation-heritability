#### VMR PCA + GLM/LMM Association Analysis ####
##
...
##
## Run from: environmental-analysis/all_individuals/tissue_compare/
...
POP_DIR      <- "all_individuals"
DATA_SUFFIX  <- "all"                # vmr_env_assoc-{DATA_SUFFIX}.tsv.gz
...
load_pheno <- function(tissue) {
  pheno_path <- here::here("inputs", "phenotypes", "_m", "phenotypes-all.tsv")
...
## Output directories
out_dir <- file.path(DRFE_DIR)   # results go alongside drfe summary files
...
for (tissue in TISSUES) {
  ...
  # Load dRFE summary for this tissue
  ...
  # Load methylation + phenotype merged data
  data_path <- here::here(
  ...
  # Load phenotype (with recoded variables) for this tissue
  ...
  for (h2_cat in H2_CATS) {
    ...
    for (env_var in ENV_VARS) {
      # Retrieve dRFE best_n_features for this task
      ...
      # Load ranked features for this task
      feat_dir <- file.path(DRFE_DIR,
                            paste0(tissue, "_", h2_safe, "_", env_var))
      feat_path <- file.path(feat_dir, "ranked_features.tsv")
      if (!file.exists(feat_path)) next

      ranked <- read.delim(feat_path)
      selected_vmrs <- head(ranked$feature, best_n)

      # Filter matrix to selected VMRs
      ...
      # PCA — retain PCs for cumulative variance >= 80%
...
for (h2_cat in H2_CATS) {
  ...
  for (env_var in ENV_VARS) {
    # Collect selected VMRs from all tissues for this (h2_cat, env_var)
    tissue_vmrs <- list()
    tissue_dfs  <- list()
    ...
    # Union of selected VMRs across tissues; scale within tissue then stack
    ...
    combined_list <- lapply(names(tissue_dfs), function(tis) {
      ...
      # Add missing VMRs from union as NA columns
      ...
      # Impute then scale within tissue
      X_mat   <- impute_median(X_mat)
      X_scaled <- scale(X_mat)
    ...
    # Load phenotype for all tissues (no tissue filter)
