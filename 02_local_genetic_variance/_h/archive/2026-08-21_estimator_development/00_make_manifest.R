...
low_h2_max <- as_num(settings$low_h2_max, "low_h2_max")
if (low_h2_max < 0 || !any(h2_values <= low_h2_max)) {
    stop("low_h2_max must be nonnegative and include at least one h2 grid level")
}
...
manifest$low_h2_max <- low_h2_max
...
manifest <- manifest[, c(
    ...
    "max_features", "raw_metric", "null_alpha", "low_h2_max",
    "max_design_distance"
)]
