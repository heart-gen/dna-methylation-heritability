...
#SBATCH --job-name=cal_h2_combine
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=1
#SBATCH --mem=24G
#SBATCH --time=03:00:00
#
# 02_local_genetic_variance: combine and QC the observed runs.
#
# The migrated step_6_combine_observed.sh handles exactly one observed run and
# is given its paths by the legacy submitter. The active workflow produces one run per arm x
...
#   ../_h/combine_observed.sh RUN_ID [RUN_ID...]
#
...
gather <- function(pattern) {
    tables <- lapply(run_ids, function(run_id) {
        dir <- file.path(analysis_dir, "_m", "runs", run_id,
                         "results", "combined")
...
show <- c("region", "population", "vmr_set_id", "expected_tasks", "analyzed_tasks",
          "expected_initial_chunks", "normally_completed_chunk_attempts",
          "interrupted_chunk_attempts", "qc_failed_tasks",
          "computational_failed_tasks", "within_domain_rate",
