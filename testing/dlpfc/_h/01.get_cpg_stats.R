...
write_covar <- function(BSobj, pheno, id, meth_merged, out_covs) {
    ...
    covar_merged |>
        write.table(file=out_cov, sep="\t", row.names=TRUE,
                    col.names=FALSE, quote=FALSE)
    qcovar_merged <- meth_selected |> select(IID, agedeath, snpPC1, snpPC2, snpPC3, snpPC4, snpPC5)
    qcovar_merged |>
...
## Main
                                        # load data
load(here("inputs/wgbs-data/dlpfc", paste0("dlpfc_chr", chr, "BSobj_GenotypesDosage.rda")))
output_path <- here("testing", "dlpfc", "_m")
subdirs <- c("covs", "cpg")
