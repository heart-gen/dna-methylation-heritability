...
for filepath in glob.glob(os.path.join(base_dir, file_pattern), recursive=True):
    try:
        ...
        if not l2_row.empty:
            ...
            # Extract annotation per row BEFORE subsetting columns
            l2_row["annotation"] = l2_row.iloc[:, 0].str.extract(r"(ANNOT_Q\d+L2_1)")

            l2_row = l2_row[["annotation", "Coefficient_z-score"]].copy()
...
if rows:
    ...
    # Save
    combined_df.to_csv("combined_ANNOT_Q_L2_1_Coeff_zscore_BA_only.tsv", sep="\t", index=False)
    print("Done! Saved combined_ANNOT_Q_L2_1_Coeff_zscore_BA_only.tsv")
else:
    print("No matching ANNOT_Q#L2_1 rows found.")

# --- Step 1: Load your summary table ---
input_file = "combined_ANNOT_Q_L2_1_Coeff_zscore_BA_only.tsv"  # change this to your file path
...
output_file = "summary_table_fdr_BA_only.tsv"
