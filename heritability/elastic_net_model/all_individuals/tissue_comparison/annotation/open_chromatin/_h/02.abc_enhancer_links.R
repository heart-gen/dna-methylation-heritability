...
for (pop in POPULATIONS) {
  ...
  fwrite(abc_links, file.path(OUT_DIR, paste0("abc_vmr_gene_links_", pop, ".tsv")), sep = "\t")
  ...
  for (t in names(gene_lists)) {
    ...
    writeLines(gene_lists[[t]]$Heritable,
      file.path(OUT_DIR, paste0("target_genes_heritable_", t_safe, "_", pop, ".txt")))
    writeLines(gene_lists[[t]]$NonHeritable,
      file.path(OUT_DIR, paste0("target_genes_nonheritable_", t_safe, "_", pop, ".txt")))
  }
  ...
  writeLines(genes_q5, file.path(OUT_DIR, paste0("target_genes_q5_pooled_", pop, ".txt")))
  writeLines(genes_q1q4, file.path(OUT_DIR, paste0("target_genes_q1q4_pooled_", pop, ".txt")))
  ...
  fwrite(abc_score_summary, file.path(OUT_DIR, paste0("abc_score_summary_", pop, ".tsv")), sep = "\t")
  ...
  for (nm in names(enrich_results)) {
    ...
    if (!is.null(er$go)) {
      ...
      if (nrow(go_df) > 0) {
        go_df$comparison <- nm
        fwrite(go_df, file.path(OUT_DIR, paste0("enrichr_go_bp_", nm_safe, "_", pop, ".tsv")),
              sep = "\t")
      }
    }
    ...
    if (!is.null(er$kegg)) {
      ...
      if (nrow(kegg_df) > 0) {
        ...
        fwrite(kegg_df, file.path(OUT_DIR, paste0("enrichr_kegg_", nm_safe, "_", pop, ".tsv")),
              sep = "\t")
      }
    }
  }
  ...
  if (nrow(go_all) > 0) {
    fwrite(go_all, file.path(OUT_DIR, paste0("enrichr_go_bp_all_", pop, ".tsv")), sep = "\t")
  ...
  if (nrow(kegg_all) > 0) {
    fwrite(kegg_all, file.path(OUT_DIR, paste0("enrichr_kegg_all_", pop, ".tsv")), sep = "\t")
