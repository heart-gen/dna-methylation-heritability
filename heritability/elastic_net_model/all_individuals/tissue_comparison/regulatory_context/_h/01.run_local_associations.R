...
run_association_pass <- function(
  ...
  links <- links |> mutate(vmr_set = vmr_set)
  ...
  vmr_summary <- assoc |>
    group_by(cohort = cohort, tissue, population, vmr_set,
             modality = modality_label, vmr_id,
...
arg6       <- ifelse(length(args) >= 6, args[[6]], NA_character_)
vmr_set    <- "shared"
max_pairs  <- NA_integer_
if (length(args) >= 7) {
  vmr_set   <- args[[6]]
  max_pairs <- if (!toupper(args[[7]]) %in% c("", "NA", "NULL", "NONE")) {
    as.integer(args[[7]])
  } else {
    NA_integer_
  }
} else if (!is.na(arg6)) {
  maybe_set <- normalize_vmr_set(arg6)
  if (!is.na(maybe_set)) {
    vmr_set <- arg6
  } else {
    max_pairs <- if (!toupper(arg6) %in% c("", "NA", "NULL", "NONE")) {
      as.integer(arg6)
    } else {
      NA_integer_
    }
...
if (modality == "expression") {
  out_dir_abc <- regctx_output_dir(
    cohort, tissue, population, "expression", "abc", vmr_set
  )
  enet_links <- load_enet(cohort, tissue, population, vmr_set) |>
    mutate(vmr_id = paste(seqnames, start, end, sep = "_")) |>
    dplyr::select(vmr_id, h2_category, h2_unscaled)
  links_abc <- load_abc_links(cohort, population) |>
    filter(tissue == !!tissue) |>
    dplyr::select(-any_of(c("h2_category", "h2_unscaled"))) |>
    inner_join(enet_links, by = "vmr_id") |>
  ...
  links_ng <- tryCatch(
    load_nearest_gene_links(cohort, tissue, population, window, vmr_set) |>
  ...
  if (nrow(links_ng) > 0) {
    out_dir_ng <- regctx_output_dir(
      cohort, tissue, population, "expression",
      paste0("nearest_gene_window_", window / 1000, "kb"), vmr_set
    )
  ...
  out_dir <- regctx_output_dir(
    cohort, tissue, population, modality,
    paste0("window_", window / 1000, "kb"), vmr_set
  )
  dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

  links <- load_psi_links(cohort, tissue, population, window, vmr_set) |>
