...
vmr_set <- ifelse(length(args) >= 7, args[[7]], "shared")
...
for (run_tag in run_tags) {
  ...
  arch <- load_architecture_covariates(cohort, tissue, population, vmr_set) |>
  ...
  if (nrow(model_results) > 0) {
    model_results <- model_results |>
      mutate(
        ...
        population = population, vmr_set = vmr_set, run_tag = run_tag
      )
  }
