...
Key fix: bitarray.decode() returns an iterator in Python 3 (not a list as in Python 2).
The iterator must be converted to a list before passing to np.array().
"""
...
class PlinkBEDFile(__GenotypeArrayInMemory__):
    ...
    def nextSNPs(self, b, minorRef=None):
        ...
        # FIX: In Python 3, bitarray.decode() returns an iterator, not a list.
        # Must convert to list before passing to np.array().
