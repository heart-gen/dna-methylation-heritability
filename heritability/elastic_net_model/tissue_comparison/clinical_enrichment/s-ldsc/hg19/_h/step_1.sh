...
SMOKING_GWAS="../EVER_SMOKER_GWAS_MA_UKB+TAG.txt.gz"
...
# --- Substance Abuse ---
# Source: Martínez-Magaña et al. 2021 (build37 formatted GWAS Catalog file)
# Build handling: this specific file is build37/hg19, so no liftover is required.
log_message "Processing: Substance Abuse (substance_abuse)"
SUBSTANCE_ABUSE_GWAS="../EVER_SUBSTANCE_ABUSE_GWAS_MA_UKB+TAG.txt.gz"
if [[ -f "$SUBSTANCE_ABUSE_GWAS" ]]; then
    run_munge python "$LDSC_WRAPPER" "$LDSC_DIR" "$LOCAL_MUNGE" \
        --sumstats "$SUBSTANCE_ABUSE_GWAS" \
    ...
    log_message "WARNING: Substance Abuse GWAS not found (${SUBSTANCE_ABUSE_GWAS}). Skipping until download completes."
fi
