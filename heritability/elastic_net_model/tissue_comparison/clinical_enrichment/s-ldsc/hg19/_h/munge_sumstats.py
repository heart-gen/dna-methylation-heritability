#!/usr/bin/env python
"""
munge_sumstats.py - Python 3 / pandas 2.x compatible version

This is a modernized version of LDSC's munge_sumstats.py script with fixes for:
1. delim_whitespace deprecation -> sep=r'\s+'
2. traceback.format_exc() signature fix
3. Other Python 3 compatibility improvements

Original: (C) 2014-2019 Brendan Bulik-Sullivan and Hilary Finucane
...
default_cnames = {
    ...
    'RS_NUMBER': 'SNP',
    'RS_NUMBERS': 'SNP',
    'VARIANT_ID': 'SNP',  # Added for imputed GWAS format
    # NUMBER OF STUDIES
    'NSTUDY': 'NSTUDY',
    'N_STUDY': 'NSTUDY',
    'NSTUDIES': 'NSTUDY',
    ...
    'SAMPLE_SIZE': 'N',  # Added for imputed GWAS format
    # SIGNED STATISTICS
    'ZSCORE': 'Z',
    'Z-SCORE': 'Z',
    'GC_ZSCORE': 'Z',
    ...
    'FRQ_U': 'FRQ',
    'F_U': 'FRQ',
    'FREQUENCY': 'FRQ',  # Added for imputed GWAS format
}
...
def parse_dat(dat_gen, convert_colname, merge_alleles, log, args):
    ...
    for block_num, dat in enumerate(dat_gen):
        ...
        tot_snps += len(dat)
        old = len(dat)
        dat = dat.dropna(axis=0, how="any", subset=[c for c in dat.columns if c != 'INFO']).reset_index(drop=True)
...
def munge_sumstats(args, p=True):
    ...
    try:
        ...
        # FIXED: traceback.format_exc() takes no positional arguments in Python 3
        # The original code passed the exception object as 'limit', which is wrong
