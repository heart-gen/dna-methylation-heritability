...
TISSUE_ORDER <- c("DLPFC", "Caudate", "Hippocampus", "Pooled")
...
fisher_df  <- fread(file.path(IN_DIR, "fishers_heritable_vs_nonheritable.tsv"))
logistic_df <- fread(file.path(IN_DIR, "logistic_continuous_h2.tsv"))
quintile_df <- fread(file.path(IN_DIR, "quintile_open_chromatin.tsv"))
vmr_df      <- fread(file.path(IN_DIR, "intergenic_vmr_atac_overlap.tsv"))

go_file <- file.path(IN_DIR, "enrichr_go_bp_all.tsv")
go_df   <- if (file.exists(go_file)) fread(go_file) else NULL

abc_file <- file.path(IN_DIR, "abc_vmr_gene_links.tsv")
...
save_plot(fig1, "fig1_celltype_heatmap", width = 5.0, height = 3.5)
...
save_plot(fig2, "fig2_forest_plot", width = 8.0, height = 3.5)
...
save_plot(fig3, "fig3_quintile_open_chromatin", width = 7.5, height = 3.2)
...
save_plot(fig4, "fig4_celltype_specificity", width = 6.5, height = 3.2)
...
if (!is.null(go_df) && nrow(go_df) > 0) {
  ...
  save_plot(fig5, "fig5_go_dotplot", width = 6.5, height = 5.5)
} else {
...
save_plot(fig_main, "main_fig_cre_enrichment", width = 12.5, height = 3.8)
...
save_plot(supp_top, "supp_fig_forest_celltype", width = 14.5, height = 3.8)
