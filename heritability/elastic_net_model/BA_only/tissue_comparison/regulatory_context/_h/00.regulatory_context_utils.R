...
load_enet <- function(cohort, tissue, population = "AA") {
  if (cohort == "BA_only") {
    ...
    safe_read(fn) |>
      ...
      dplyr::select(seqnames, start, end, h2_category, h2_unscaled,
                    r_squared_cv, num_snps, population)
  } else {
    fn <- here("heritability", "elastic_net_model", "all_individuals", tissue, "_m",
...
load_nearest_gene_links <- function(cohort, tissue, population = "AA",
  ...
  has_h2 <- all(c("h2_category", "h2_unscaled") %in% colnames(df))
  if (!has_h2) {
    enet <- load_enet(cohort, tissue, population) |>
