...
#SBATCH --job-name=libd_eqtl_tqtl
#SBATCH --output=logs/libd_eqtl_tqtl.%j.log

# TensorQTL cis map (genes, LIBD AA-only standard covariates)
# Submit from _m/ after step_1:
#   sbatch --dependency=afterok:<prep> --export=ALL,REGION=caudate ../_h/step_2.sh
...
PREFIX="libd_aa_${REGION}_genes_standard"
DEVICE="${DEVICE:-gpu}"
...
mkdir -p "${TQTL}"
log_message "LIBD AA tensorQTL cis ${REGION} genes (device=${DEVICE})"
head -1 "${COV}"
