#!/usr/bin/env python3
"""Build shared-donor-aware caudate downsample sample lists (Phase 4 Experiment 3).
...
Mirrors 08_schizophrenia_risk_application/_h/08_make_caudate_downsample_lists.py
but writes under 04_cross_region_sharing/_m/caudate_downsample/.
"""
...
def main() -> None:
    ...
    write_tsv(outdir / "downsample_design_summary.tsv", [{
        ...
        "method_note": (
            "Lead-SNP retention under N-matched downsample (not full cis remapping per replicate); "
            "same design as Phase 7 Tier A"
        ),
    }])
